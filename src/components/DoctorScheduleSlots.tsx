...
import { getOrCreateConversation } from "@/services/appointmentService";
                                        ...
                                        <div className="flex-1 flex justify-end gap-1.5 overflow-x-auto no-scrollbar">
                                            ...
                                            {slot.isBooked && (
                                                slot.appointmentStatus === "COMPLETED" ? (
                                                    <button
                                                        onClick={async () => {
                                                            if (slot.conversationId) {
                                                                router.push(`/chat?id=${slot.conversationId}`);
                                                            } else if (slot.appointmentId) {
                                                                try {
                                                                    setLoading(true);
                                                                    const convo = await getOrCreateConversation(slot.appointmentId);
                                                                    router.push(`/chat?id=${convo.id}`);
                                                                } catch (error) {
                                                                    console.error("Failed to get/create conversation:", error);
                                                                    setLoading(false);
                                                                    showToast("Không thể mở cuộc hội thoại!");
                                                                }
                                                            }
                                                        }}
                                                    ...
                                                    /* No conversationId yet */
                                                    isSlotPast(slot.availableDate, slot.startTime) ? (
                                                        /* If past but not completed yet - maybe show nothing or just completed button if possible */
                                                        isAppointmentEnded(slot.availableDate, slot.startTime) && (
                                                            <button
                                                                onClick={() => handleComplete(slot)}
                                                                className="flex items-center gap-1.5 rounded-xl bg-emerald-600 px-3 py-1.5 text-[11px] font-bold text-white hover:bg-emerald-700 transition shadow-sm shadow-emerald-100 whitespace-nowrap"
